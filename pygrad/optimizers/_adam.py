...
class Adam(Gradient):
    ...
    def __init__(
        ...
        self._beta1 = beta1
        self._beta2 = beta2
        self._moment1 = [np.zeros_like(p.value) for p in parameters]
        self._moment2 = [np.zeros_like(p.value) for p in parameters]
    ...
    def _update(self, learning_rate: float):
        ...
        for param, m1, m2 in zip(
            ...
            param._value += alpha * m1 / (np.sqrt(m2) + np.finfo(g.dtype).eps)
