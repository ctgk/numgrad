...
def test_forward(x, y, name):
    actual = pg.add(x, y, name=name)
    assert np.allclose(actual.value, np.add(x, y))
...
@pytest.mark.parametrize('x, y, dz, expected_dx, expected_dy', [
    (
        ...
        pg.Array(3., is_differentiable=True), None, np.ones(3), 3
    ),
    (
        5, pg.Array([-7., 3], is_differentiable=True),
...
def test_numerical_grad(x, y):
    (x + y).backward()
    dx, dy = _numerical_grad(pg.add, x, y, epsilon=1e-3)
